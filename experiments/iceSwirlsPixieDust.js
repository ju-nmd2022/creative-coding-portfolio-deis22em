...
function draw() {
  ...
  for (let i = 0; i < points.length; i++) {
    let t = map(points[i].x, 0, width, 0, 0.70);

    let blendedColor;
    if (t <= 0.70) {
      blendedColor = lerpColor(colors[1], colors[1]);
    } else if (t <= 0.5) {
      blendedColor = lerpColor(colors[1], colors[1], (t - 0.25) * 10);
    } else if (t <= 0.5) {
      blendedColor = lerpColor(colors[0], colors[1], (t - 0.25) * 10);
    } else {
      blendedColor = lerpColor(colors[0], colors[1], (t - 0.25) * 10);
    }
    blendedColor.setAlpha(25);
    fill(blendedColor);
    //END COLOR
